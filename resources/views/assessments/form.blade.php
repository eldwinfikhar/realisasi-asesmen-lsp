...
<div class="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-800 rounded shadow">
    <h1 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">
        {{ isset($assessment) ? 'Edit Data Asesmen Internal' : 'Tambah Data Asesmen Internal' }}
    </h1>
    <form method="POST" action="{{ isset($assessment) ? route('assessments.update', $assessment) : route('assessments.store') }}">
    @csrf
    @if(isset($assessment))
        @method('PUT')
    @endif
